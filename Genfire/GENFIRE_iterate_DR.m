...
function [rec, errK, Rfree_complex] = GENFIRE_iterate_DR(numIterations,initialObject,support,measuredK,constraintIndicators,constraintEnforcementDelayIndicators,R_freeInd_complex,R_freeVals_complex, enforce_positivity, enforce_support)
if nargin < 9
    enforce_support = 1;
...
for iterationNum = 1:numIterations
    ...
    fprintf('Iteration %d. ErrK = %.5f\n',iterationNum, errK(iterationNum));
    %enforce Fourier constraint
    k(constraintInd_complex) = measuredK(constraintInd_complex);
    
    u_K = (my_ifft(k));%obtain next object with IFFT
    initialObject = 2*u_K - u;
    
    %method1
    initialObject = real(initialObject);
    index_neg = initialObject<0 | ~support;
